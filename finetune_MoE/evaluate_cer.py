...
def run_evaluation(model, processor, eval_dataset, args):
    ...
    # 计算 CER
    cer = jiwer.cer(list(data1["reference_clean"]), list(data1["hypothesis_clean"]))
    return cer * 100

# 主程式部分
if __name__ == "__main__":
    # 模型权重路径，指向你找到的完好的checkpoint
    model_path = "/mnt/lv3/aiyuchen/models/whisper/large-v3"
    
    # 处理器路径，指向你训练时使用的、文件完整的原始基础模型
    # 从你的训练脚本中可以看到这个路径
    processor_path = "/mnt/lv3/aiyuchen/models/whisper/large-v3" 
    ...
    language_mapping = {
        ...
        "my_mm": "burmese", "ceb_ph": "cebuano", "km_kh": "khmer",
        "lo_la": "lao",
    }
    ...
    parser.add_argument("--max_new_tokens", default=444, type=int)
    ...
    parser.add_argument("--save_path", default="eval_results/large-v3", help="保存评估结果的目录")

    args = parser.parse_args()
    print(f"Settings: {args}")
    os.makedirs(args.save_path, exist_ok=True)

    output_csv  = os.path.join(args.save_path, "eval_cer_summary_ext.csv")
    output_json = os.path.join(args.save_path, "eval_cer_summary_ext.json")
    ...
    for single_dataset_setting in all_datasets_settings:
        ...
        # 对当前语言的数据集进行评估
        cer = run_evaluation(model, processor, ds["test"], args)

        # 打印当前语言的结果
        print(f"✅ 语言 {lang_abbr} 的 CER: {cer:.2f} %")
        print("--- 评估结束 ---\n")
        
        all_lang_results.append({"language": lang_abbr,
                                 "cer": round(float(cer), 2),
