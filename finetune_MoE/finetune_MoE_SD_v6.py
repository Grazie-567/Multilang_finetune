...
def replace_ffn_with_deepspeed_moe(model, config, moe_config):
    ...
    def create_moe_wrapper(original_layer, expert_count, top_k_experts):
        ...
        num_languages = 100
        layer_lang_embed = nn.Embedding(num_languages, config.d_model)

        layer_lang_proj = nn.Linear(config.d_model, num_experts)
        
        return moe, layer_expert_bias, layer_lang_embed, layer_lang_proj
    ...
    for idx in moe_layers:
        ...
        # 创建MoE并替换
        layer.moe_layer, layer.expert_bias, layer.lang_embed, layer.lang_proj = create_moe_wrapper(layer, num_experts, top_k)
        ...
        def moe_forward(
            ...
            # ===== language-aware bias =====
            if hasattr(self, "current_lang_id") and self.current_lang_id is not None:
                lang_emb = self.lang_embed(self.current_lang_id)  # [B, D]
                lang_emb = lang_emb.unsqueeze(1).expand(-1, seq_len, -1)
                lang_emb = lang_emb.reshape(-1, hidden_dim)       # [B*T, D]

                lang_bias_logits = 0.1 * self.lang_proj(lang_emb)       # [1, num_experts]
                lang_bias = torch.matmul(lang_bias_logits, self.expert_bias)  # [1, D]

                moe_output = moe_output + lang_bias
            else:
                bias = self.expert_bias.mean(dim=0)
                moe_output = moe_output + bias
            
            # ===== 加入 expert diversity =====
            bias = self.expert_bias.mean(dim=0)   # minimal侵入：不改router
            hidden_states_2d = moe_output + bias
...
class DeepSpeedMoETrainer(Seq2SeqTrainer):
    ...
    """
    def __init__(self, probs, teacher_model=None, old_language="en", tokenid2langid=None, *args, **kwargs):
        ...
        self.probs = probs
        self.tokenid2langid = tokenid2langid
    ...
    def compute_loss(
        ...
        **kwargs
        ...
        """
        def inject_lang_id(module, lang_ids):
            for layer in module.model.encoder.layers:
                layer.current_lang_id = lang_ids
            for layer in module.model.decoder.layers:
                layer.current_lang_id = lang_ids

        raw_lang_ids = inputs["labels"][:, 0]
        lang_ids = torch.tensor(
            [self.tokenid2langid.get(int(x), 0) for x in raw_lang_ids],
            device=raw_lang_ids.device
        )
        ...
        inject_lang_id(base_model, lang_ids)
        ...
        # language-aware routing 约束
        lang_reg_loss = 0.0
        # 防止路由崩溃
        lang_div_loss = 0.0

        for layer in base_model.model.encoder.layers:
            if hasattr(layer, "lang_embed"):
                lang_reg_loss += layer.lang_embed.weight.norm(p=2)
            if hasattr(layer, "lang_proj"):
                logits = layer.lang_proj(layer.lang_embed.weight)  # [L, num_experts]
                probs = torch.softmax(logits, dim=-1)
                entropy = -(probs * torch.log(probs + 1e-8)).sum(dim=-1).mean()
                lang_div_loss += entropy
        ...
        loss += 1e-4 * lang_reg_loss
        loss -= 0.001 * lang_div_loss
        
        # distillation loss
        if teacher_logits is not None:
            
            distill_lambda = self.distill_lambda * (1 - 0.9 * progress)
            hidden_lambda = self.hidden_lambda * (1 - 0.9 * progress)
            lang_lambda = self.lang_lambda * (1 - 0.9 * progress)
...
def get_param_groups(model):
    ...
    return [
        {"params": base_params, "lr": args.learning_rate * 0.3},
        {"params": expert_params, "lr": args.learning_rate * 1.0},
        {"params": gate_params, "lr": args.learning_rate * 1.2},
    ]
...
if __name__ == "__main__":
    ...
    lang_tokens = [
        t for t in tokenizer.get_vocab().keys()
        if t.startswith("<|") and t.endswith("|>") and len(t) <= 6
    ]

    lang2id = {t: i for i, t in enumerate(sorted(lang_tokens))}
    id2lang = {i: t for t, i in lang2id.items()}

    if rank == 0:
        tokenid2langid = {
            tokenizer.convert_tokens_to_ids(t): i
            for t, i in lang2id.items()
        }
    else:
        tokenid2langid = None
    
    obj_list = [tokenid2langid]
    torch.distributed.broadcast_object_list(obj_list, src=0)
    tokenid2langid = obj_list[0]
    
    assert tokenid2langid is not None
    ...
    trainer = DeepSpeedMoETrainer(
        ...
        probs=probs,
        tokenid2langid = tokenid2langid,
        optimizers=(torch.optim.AdamW(param_groups), None),
    )
