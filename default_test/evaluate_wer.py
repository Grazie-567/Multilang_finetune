...
if __name__ == "__main__":
    ...
    model_path = "/mnt/lv2/FLEURS/whisper-medium"
    
    # 处理器路径，指向你训练时使用的、文件完整的原始基础模型
    # 从你的训练脚本中可以看到这个路径
    processor_path = "/mnt/lv2/FLEURS/whisper-medium" 
    ...
    parser.add_argument("--save_path", default="eval_results/medium", help="保存评估结果的目录")

    args = parser.parse_args()
    print(f"Settings: {args}")
    os.makedirs(args.save_path, exist_ok=True)

    output_csv  = os.path.join(args.save_path, "eval_wer_summary_new.csv")
    output_json = os.path.join(args.save_path, "eval_wer_summary_new.json")
